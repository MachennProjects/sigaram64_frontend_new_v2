...
export default function PgnGameViewer({
  ...
  const handleStartClientAnalysis = async () => {
    ...
    try {
      ...
      const moveDetails = game.moves.map((m) => {
      ...
      const gameResultHeader = game.headers['Result'] || '*';
      const resultOutcome = 
        gameResultHeader === '1-0' ? 'white_win' : 
        gameResultHeader === '0-1' ? 'black_win' : 
        gameResultHeader === '1/2-1/2' ? 'draw' : 'draw';
      ...
      console.error('Failed to run Stockfish analysis on PGN game:', err);
    } finally {
  ...
  const currentMoveSan = currentMove ? currentMove.san : '';
  const currentMoveColor = currentMove ? currentMove.color : 'w';

  const moveCommentary = useMemo(() => {
    if (moveIndex === -1) {
      return game.startComment || "Study this game. Use keyboard Left/Right arrows or the Autoplay button to step through moves!";
    }
    if (currentMoveComment) {
      return currentMoveComment;
    }
    return getMoveCommentary(currentMoveSan, moveIndex, currentMoveColor);
  }, [moveIndex, game.startComment, currentMoveComment, currentMoveSan, currentMoveColor]);
  ...
  const currentAnalyzedMove = showAnalysis && analysisResult && moveIndex >= 0 ? analysisResult.moves[moveIndex] : null;
        ...
        {isAnalyzingClient ? (
          ...
          currentAnalyzedMove ? (
            ...
            <div className="bg-white text-[#111] rounded-xl p-4 shadow-lg flex-1 border-2 border-[#C9A84C]/80 flex items-center select-none text-left w-full">
              <span className="text-xs font-semibold text-gray-700">Let's review this game! Click on any move to start the review.</span>
            </div>
          )
        ) : (
          <div className="bg-[#1e2e52]/20 border border-[#2b3e6d]/40 rounded-xl p-4 flex-1 flex flex-col sm:flex-row justify-between items-center gap-4 w-full text-left">
            ...
            <table className="w-full table-fixed text-center border-b border-[#2B2927] pb-3 block">
              <thead className="block w-full">
                <tr className="flex w-full mb-2">
                  ...
                  <th className="w-[35%] text-[11px] font-bold text-white truncate px-1 text-center">{game.headers['White'] || 'White'}</th>
                  <th className="w-[35%] text-[11px] font-bold text-white truncate px-1 text-center">{game.headers['Black'] || 'Black'}</th>
                </tr>
              </thead>
  ...
  const bottomControls = (
    <div className="flex gap-2 p-3 border-b lg:border-t lg:border-b-0 border-[#1E2E52] order-2 lg:order-3 bg-[#1D1C1A] flex-shrink-0">
      ...
      >
        <span>{isAutoplay ? '⏸️ Pause' : '▶️ Play'}</span>
      </button>
    </div>
  );
